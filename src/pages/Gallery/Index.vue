  ...
  <Layout>
    <v-main>
      <v-container class="pa-0 background-color-fafafa" fluid>
        <div class="py-10" />
        <div id="start" class="playfair-38-700 text-center">
          {{ $helper.plainText.gallery[getLang] }}
        </div>
        <div class="py-4" />
        <!-- Desktop -->
                        ...
                        <v-col cols="auto" class="d-flex flex-column align-end">
                          <v-card-actions>
                            <v-tooltip top color="black">
                              ...
                              <span>{{
                                $helper.plainText.heart[getLang]
                              }}</span>
                            </v-tooltip>
                            <v-tooltip v-if="canCopy" top color="black">
                              ...
                              <span>{{
                                $helper.plainText.share[getLang]
                              }}</span>
                            </v-tooltip>
                            <v-tooltip top color="black">
                              <template v-slot:activator="{ on, attrs }">
                              ...
                              <span>{{
                                $helper.plainText.artworkZoom[getLang]
                              }}</span>
                            </v-tooltip>
                            <v-tooltip top color="black">
                              <template v-slot:activator="{ on, attrs }">
                              ...
                              <span>{{
                                $helper.plainText.artistPage[getLang]
                              }}</span>
                            </v-tooltip>
                          </v-card-actions>
                          <div class="pb-2 pr-4 text-end">
                            <div
                              class="raleway-23-400 d-flex justify-end align-center"
                              v-if="artwork.salePrice"
                            >
                              <v-tooltip
                                v-if="!$eshop.isInBasket(artwork.public_id)"
                                left
                                color="black"
                              >
                                <template v-slot:activator="{ on, attrs }">
                                  <v-btn
                                    ...
                                    color="#333333"
                                    @click="
                                      $eshop.addToBasket(artwork);
                                      $forceUpdate();
                                    "
                                  >
                                    <v-icon size="20" color="#FAFAFA"
                              ...
                              <v-tooltip v-else left color="black">
                                <template v-slot:activator="{ on, attrs }">
                                  <v-btn
                                    ...
                                    color="#FAFAFA"
                                    @click="
                                      $eshop.removeFromBasket(
                                        artwork.public_id
                                      );
                                      $forceUpdate();
                                    "
                                  >
                                    <v-icon color="#333333" size="20"
                                      >mdi-basket-minus</v-icon
                                    >
                                  </v-btn>
                                </template>
                                <span>{{
                            ...
                            <div
                              ...
                              v-if="artwork.rentPrice"
                            >
                              <span class="pr-1">{{
                                $helper.plainText.rentFor[getLang]
                              }}</span>
                              {{ artwork.rentPrice }}
                              <span>{{
                                $helper.plainText.rentPerMonth[getLang]
                              }}</span>
                            </div>
                          </div>
                          <div
                      ...
                      <v-card>
                        ...
                        <v-row justify="space-between" no-gutters>
                          ...
                          >
                            ...
                            <div class="pb-2 pr-4 text-end">
                              <div
                                class="raleway-16-400 d-flex justify-end align-center"
                                v-if="artwork.salePrice"
                              >
                                <v-btn
                                  v-if="
                                    true !==
                                    $eshop.isInBasket(artwork.public_id)
                                  "
                                  class="mr-2"
                                  fab
                                  height="26"
                                ...
                                <v-btn
                                  ...
                                  color="#FAFAFA"
                                  @click="
                                    $eshop.removeFromBasket(artwork.public_id);
                                    $forceUpdate();
                                  "
                                >
                                  <v-icon color="#333333" size="16"
                                    >mdi-basket-minus</v-icon
                                  >
                                </v-btn>
                                <div>{{ artwork.salePrice }}€</div>
          ...
          .then((found) => {
            if (found.total_count > 0) {
              ...
              found.resources.forEach(async (resource) => {
                if (
                  resource.folder.indexOf("approved") !== -1 &&
                  resource.folder.indexOf(
                    "google-oauth2|104266192030226467336"
                  ) === -1 &&
                  resource.folder.indexOf(
                    "google-oauth2|108119525360718636347"
                  ) === -1 &&
                  resource.folder.indexOf(
                    "google-oauth2|108119525360718636347"
                  ) === -1 &&
                  resource.folder.indexOf("facebook|10220098526787758") ===
                    -1 &&
                  resource.folder.indexOf("facebook|3306299589452085") === -1 &&
                  resource.folder.indexOf("auth0|5f5bd27a52581200703b8b00") ===
                    -1
                ) {
                  ...
                  if (artist !== null) {
                    ...
                    var size = "";
                    var type = "";
                    var tags = Object.prototype.hasOwnProperty.call(
                      resource,
                      "tags"
                    )
                      ? resource.tags
                    ...
                    var prio = 0;
                    if (
                      Object.prototype.hasOwnProperty.call(resource, "context")
                    ) {
                      // Title
                      if (
                      ...
                      if (
                        ...
                        type = this.$helper.plainText.type[
              ...
              // block, so that helper approved artworks Object local to block
              {
                ...
                approvedArtworks.forEach((artwork) => {
                  ...
                  if (
                    !Array.isArray(helperObj[artwork.artist_name][artwork.prio])
                  ) {
              ...
              this.totalPages.desktop =
                Math.floor(
                  this.approvedArtworks.length / this.artworksPerPage.desktop
                ) + 1;
              this.totalPages.mobile =
                Math.floor(
                  this.approvedArtworks.length / this.artworksPerPage.mobile
                ) + 1;
              ...
              this.approvedArtworks.forEach((artwork, index) => {
                ...
                this.gallery[
                  Math.floor(index / this.artworksPerPage.mobile)
                ].push(artwork);
  ...
  methods: {
    ...
    getUserFavorites() {
      return new Promise((resolve, reject) => {
        this.$db
          .getFavorites(this.$auth.user.sub)
          .then((favorites) => {
    ...
    toggleFavorite(artwork) {
      if (!this.$auth.isAuthenticated()) {
        ...
        const artwork_id = this.$helper.toPublicIdNoPath(
          artwork.public_id,
          "/approved/"
        );
    ...
    shareArtwork(artwork) {
      if (process.isClient) {
        navigator.clipboard
          .writeText(artwork.url)
          .then(() => {
            this.isSharing.text.gr =
              "Η διεύθυνση της εικόνας αντιγράφτηκε επιτυχώς!";
            this.isSharing.text.en = "Image URL successfully copied!";
          }) // copied
          .catch(() => {
            this.isSharing.text.gr =
              "Η αντιγραφή της διεύθυνσης της εικόνας απέτυχε.";
            this.isSharing.text.en = "Failed to copy image URL.";
          }) // not copied
          .finally(() => {
            this.isSharing.state = true;
            this.isSharing.timeoutCb = setTimeout(
              () => this.clearSharingDialog(),
              3000
            );
          });
      }
