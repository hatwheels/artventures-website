    ...
    <v-app-bar flat class="hidden-sm-and-down px-11" app absolute color="#e8e8e8" height="76px">
      ...
      <v-toolbar-items class="pa-0">
        ...
        <v-menu open-on-hover bottom :offset-y="true" transition="slide-y-transition">
          ...
          <v-list flat color="#e8e8e8">
            <v-list-item v-for="(language, i) in languages" :key="'lang-' + i">
              <v-list-item-title
                :class="getLang === 'gr' ? 'noto-15-600' : 'nunito-15-600'"
                class="color-333333 text-center text-uppercase"
                v-html="language"
                 @click.stop.prevent="selectLang(language)"
              />
            </v-list-item>
          </v-list>
        </v-menu>
              ...
              <v-carousel ref="menucarousel" hide-delimiter-background hide-delimiters :show-arrows="false">
                ...
                <v-carousel-item>
                  ...
                  <v-list v-else color="#ffffff" class="px-9 py-9">
                    ...
                    >
                      <v-list-item-title :class="getLang === 'gr' ? 'noto-35-400' : 'raleway-35-400'" @click.stop.prevent="selectLangModal(language)">
